...
def validate_quench(file_name, fault_data, time_data, saved_loaded_q, frequency):
    ...
    # fitting using log ratio of A_0 / A(t)
    log_function = np.log(pre_quench_amp / fault_data)
    best_fit_coeffs = np.polyfit(time_data, log_function, 1)
    ...
    exponential_term = best_fit_coeffs[0]
    print(f"Exponential term is {exponential_term}")
    print(f"Best fit polynomial is y={exponential_term:.2e}*t + ({best_fit_coeffs[1]:.2e})" )
